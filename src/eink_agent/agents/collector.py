...
class Collector(BaseAgent):
    ...
    def _run(self, input_data: object, trace: Trace) -> CollectorResult:
        if not isinstance(input_data, str) or not input_data.strip():
            raise ValueError("Collector 输入必须是非空 URL 字符串")
        ...
        # 1) 抓取页面 HTML
        self._push_event(trace, level="info", message="fetch_page", data={"url": url})
        resp = requests.get(
            url,
            timeout=20,
            headers={
        ...
        self._push_event(trace, level="info", message="extracted_text", data={"finalUrl": final_url, "textChars": len(text)})
