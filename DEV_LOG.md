...
### [2026-04-17 02:30] 宝玉风格双层标题 + emoji 装饰 + 标签区域滚动修复

- 需求/问题：
  - 现有标题虽然经过清洗，但语气仍偏“说明书”，缺少宝玉风格的金句感与双层视觉结构。
  - 用户希望技术类/思考类内容在标题前自动加上合适的 emoji，形成一眼可识别的语义锚点。
  - 480x800 预览卡片在标签较多时，底部标签区域会出现“被裁切/看似重叠”的问题。
- 根因分析：
  - Refiner 只产出单一 `title`，未对主/副标题做结构化约束，Publisher 只能把同一字段当作大标题使用。
  - 卡片容器的高度被固定为 `h-[800px]` 且 `overflow-hidden`，一旦内容超出就会被硬裁切。
  - 标题未结合标签/正文做“内容类型识别”，无法自动判断是技术向还是思考向。
- 修改内容：
  - `src/eink_agent/agents/refiner.py`：
    - System Prompt 升级为“宝玉风格”结构化输出：要求 LLM 返回 `main_title`（主标题/金句）与 `sub_title`（副标题/核心要点），并仍包含 `summary/tags/confidence`。
    - 解析逻辑兼容老格式：
      - 优先从 `main_title/sub_title` 取值；若缺失则回退至 `title/summary`，副标题缺失时用摘要第一句兜底。
    - 新增 `_decorate_title_with_emoji`：
      - 根据标签与正文判断内容类型：技术类（API/SDK/部署/工程/Transformer/RAG/embedding 等）加 `💻` 前缀；思考类（思考/反思/认知/洞察/启发等）加 `🧠` 前缀；
      - 若标题中已有人为添加 `💻/⚡/🧠/💡`，则不再重复。
    - fallback 逻辑同步升级：
      - 主标题由 `_clean_title_for_card` + `_repair_truncated_ascii_suffix` 生成，副标题取正文开头 80 字左右，并走同一套 emoji 装饰与置信度打分逻辑。
  - `src/eink_agent/agents/publisher.py`：
    - 标题区域改为双层视觉结构：
      - 主标题：`font-size: 2rem`、`font-weight: 800`、纯白文字，承载“金句 + emoji”。
      - 副标题：`font-size: 1.1rem`、`font-weight: 400`、`rgba(255,255,255,0.7)` 半透明灰色。
    - 在不改变外框尺寸的前提下，将最外层容器从 `overflow-hidden` 调整为 `overflow-y-auto overflow-x-hidden`，当标签较多或内容偏长时可轻微纵向滚动，避免底部标签被裁切或看似重叠。
- 验证结果：
  - 对技术类与思考类输入分别进行测试：
    - 技术向标题前自动出现 `💻`，思考向出现 `🧠`，未出现误判重复添加。
    - 主副标题在预览卡片中呈现明显层级，主标题为大号金句，副标题为一行核心补充。
  - 在标签数量较多的情况下，480x800 预览卡片底部标签区域不再被裁切，可通过微小滚动完整查看所有标签。
- 风险与回滚点：
...
- 需求/问题：
  ...
  - 技术类/思考类内容的标题缺乏一眼可辨的情绪/语境提示。
  - 480x800 预览卡在标签较多时，底部标签行会被裁切，看起来像“重叠”。
- 修改内容：
  - `src/eink_agent/agents/refiner.py`：
    - System Prompt 升级为请求结构化标题：
      - `main_title`：主标题/金句（建议 8–12 字），禁止口水化开头，鼓励“对象 + 关键结论/能力”或对比结构。
      - `sub_title`：副标题（15–20 字），对主标题做一行解释或补充价值点。
    - 解析逻辑兼容老数据：
      - 优先读取 `main_title/sub_title`；若缺失则回退到 `title/summary`，再退到“摘要首句”。
      - 将主/副标题映射回现有 Schema：`title=主标题`，`summary=副标题`。
    - 新增 `_decorate_title_with_emoji`：
      - 根据正文与标签中的关键词粗分“技术类/思考类”：
        - 技术向：在标题前加 `💻`。
        - 思考向：在标题前加 `🧠`。
      - 若标题中已有人为加过 `💻/⚡/🧠/💡`，则不重复添加。
    - fallback 路径同步接入上述逻辑：在本地兜底生成主/副标题时，同样进行标题清洗与 emoji 装饰。
  - `src/eink_agent/agents/publisher.py`：
    - 预览卡片标题区域视觉层级调整：
      - 主标题使用 `font-size: 2rem` + `font-extrabold` + 纯白色，作为金句。
      - 副标题使用 `font-size: 1.1rem` + 普通字重 + `rgba(255,255,255,0.7)`，承接主标题。
      - 保留左侧 4px 渐变竖条作为“金句”装饰锚点。
    - 预览容器由 `overflow-hidden` 改为 `overflow-y-auto overflow-x-hidden`，在高度内容较多（尤其标签过多）时允许竖向轻微滚动，避免底部标签被裁切而产生“重叠错位”观感。
- 验证结果：
  - 使用技术向/思考向示例文本回归：
    - 技术类文本标题自动加上 `💻` 前缀，思考类示例可触发 `🧠` 前缀，且不会与手动添加 emoji 冲突。
    - 预览卡片标题区域明显呈现“主标题（金句）+ 副标题（补充）”双层结构。
  - 当 tags 达到 4–5 个时，底部标签区不再被裁切；必要时可以在卡片内部轻微滚动查看完整内容。
- 风险与回滚点：
